...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        if (world_rank == 0) {
            fprintf(stderr, "Usage: mpirun -np <p> ./cannon_mm <N> [seed]\n");
            fprintf(stderr, "  N    = global matrix dimension (NxN)\n");
    ...
    MPI_Scatter(A_packed, nb * nb, MPI_DOUBLE,
                A_local, nb * nb, MPI_DOUBLE,
                0, MPI_COMM_WORLD);

    MPI_Scatter(B_packed, nb * nb, MPI_DOUBLE,
                B_local, nb * nb, MPI_DOUBLE,
                0, MPI_COMM_WORLD);

    MPI_Barrier(MPI_COMM_WORLD);
    double t0 = MPI_Wtime();

    cannon_multiply(A_local, B_local, C_local, nb, q, pr, pc, MPI_COMM_WORLD);
    ...
    MPI_Gather(C_local, nb * nb, MPI_DOUBLE,
               C_packed, nb * nb, MPI_DOUBLE,
               0, MPI_COMM_WORLD);

    MPI_Scatter(C_ref_packed, nb * nb, MPI_DOUBLE,
                C_ref_local, nb * nb, MPI_DOUBLE,
                0, MPI_COMM_WORLD);
